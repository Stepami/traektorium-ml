...
class Topics(Resource):
    def get(self, topics_num, top_words_num):
        processed_text = list(map(lambda obj: obj['description'], read_from_json()))

        vectorizer = TfidfVectorizer()
        features = vectorizer.fit_transform(processed_text)

        nmf = NMF(n_components=topics_num, init='random', random_state=0, max_iter=1000)
        nmf.fit(features)
...
parser = api.parser()
parser.add_argument('text', type=str, location='json', help='Text to find neighbors of')
...
class Neighbors(Resource):
    ...
    def post(self, neighbors_num):
        ...
        vectorizer = TfidfVectorizer()
        features = vectorizer.fit_transform(list(map(lambda obj: obj['description'], data)))
